...
const listPages = (times) => new Array(times).fill(0).map((_, index) => `<div id='Page${index + 1}' class='MangaPage'>
...
const listOptions = (times) => new Array(times).fill(0).map((_, index) => `<option value='${index + 1}'>${index + 1}</option>`);
const listThumbnails = (times) => new Array(times).fill(0).map(
  (_, index) => `<div id='Thumbnail${index + 1}' class='Thumbnail'><img id='ThumbnailImg${index
